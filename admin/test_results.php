...
// Get all candidates who have taken tests
$candidates = $pdo->query("
    SELECT c.id, c.name, c.email, COUNT(tr.id) AS total_questions
    FROM candidates c
    JOIN test_results tr ON c.id = tr.candidate_id
    GROUP BY c.id
")->fetchAll();

// Get specific candidate results if selected
$results = [];
if (isset($_GET['candidate_id'])) {
    $stmt = $pdo->prepare("
        SELECT q.question, q.type, q.answer AS correct_answer, tr.answer AS candidate_answer
                  ...
                  <div class="card">
                    ...
                    <div class="table-responsive text-nowrap">
                      <table class="table table-hover">
                        <thead>
                          <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase" onclick="sortTable(0)">Candidate</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase" onclick="sortTable(1)">Email</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase" onclick="sortTable(2)">Questions Attempted</th>
                        ...
                        <tbody class="divide-y divide-gray-200" id="tableBody">
                          ...
                          <tr>
                            <td class="px-6 py-4"><?= htmlspecialchars($c['name']) ?></td>
                            <td class="px-6 py-4"><?= htmlspecialchars($c['email']) ?></td>
                            <td class="px-6 py-4"><?= $c['total_questions'] ?></td>
                            <td class="px-6 py-4">
                                <a href="test_results.php?candidate_id=<?= $c['id'] ?>" 
                    ...
                    <div class="bg-white rounded-lg shadow p-6">
                        <h2 class="text-xl font-bold mb-4">Test Details for <?= htmlspecialchars($results[0]['name']) ?></h2>
                        
                        <div class="space-y-6">
                            <?php foreach ($results as $index => $result): ?>
                            <div class="border rounded p-4">
                                <div class="font-semibold mb-2">
                                    Question <?= $index+1 ?> (<?= strtoupper($result['type']) ?>)
                                </div>
                                <div class="mb-4"><?= htmlspecialchars($result['question']) ?></div>
                                
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <div class="text-sm text-gray-600 mb-1">Candidate's Answer:</div>
                                        <div class="p-2 bg-gray-50 rounded">
                                            <?= htmlspecialchars($result['candidate_answer']) ?>
                                        </div>
                                    </div>
                                    <div>
                                        <div class="text-sm text-gray-600 mb-1">Correct Answer:</div>
                                        <div class="p-2 bg-green-50 rounded">
                                            <?= htmlspecialchars($result['correct_answer']) ?>
                                        </div>
                                    </div>
                                </div>
                                
                                <?php if ($result['type'] === 'mcq'): ?>
                                <div class="mt-2">
                                    <?php if (trim($result['candidate_answer']) === trim($result['correct_answer'])): ?>
