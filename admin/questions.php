...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_question'])) {
    ...
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM questions WHERE type = ?");
    $stmt->execute([$type]);
    $count = $stmt->fetchColumn();

    $max = ($type === 'mcq') ? 60 : 40;
    if ($count >= $max) {
        $_SESSION['error'] = "Maximum $max $type questions reached!";
        header("Location: questions.php");
        exit();
    }

    $stmt = $pdo->prepare("INSERT INTO questions (question, type, options, answer) VALUES (?, ?, ?, ?)");
    if ($stmt->execute([$question, $type, $options, $answer])) {
...
// Fetch all questions
$stmt = $pdo->query("SELECT * FROM questions ORDER BY type");
...
<html
  ...
  <head>
    ...
    <title>Exam Questions | SoT</title>
                        ...
                        <div class="card-body">
                          <form method="POST">
                            ...
                            <div id="options-container" class="mb-4">
                                ...
                                <div class="space-y-2">
                                    <input type="text" name="options[]" class="w-full mb-2 p-2 border rounded" placeholder="Option 1">
                                    <input type="text" name="options[]" class="w-full mb-2 p-2 border rounded" placeholder="Option 2">
                                    <input type="text" name="options[]" class="w-full mb-2 p-2 border rounded" placeholder="Option 3">
                                    <input type="text" name="options[]" class="w-full mb-2 p-2 border rounded" placeholder="Option 4">
                                </div>
                            </div>
                  ...
                  <div class="card">
                    ...
                    <div class="table-responsive text-nowrap">
                      <table class="table table-hover">
                        <thead>
                          <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase" onclick="sortTable(0)">ID</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase" onclick="sortTable(1)">Type</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase" onclick="sortTable(2)">Question</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase" onclick="sortTable(3)">Options</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase" onclick="sortTable(4)">Answer</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase" onclick="sortTable(5)">Actions</th>
                        </thead>
                        <tbody class="divide-y divide-gray-200" id="tableBody">
                          <?php foreach ($questions as $q): ?>
                          <tr>
                            <td class="px-6 py-4"><?= $q['id'] ?></td>
                            <td class="px-6 py-4 uppercase"><?= $q['type'] ?></td>
                            ...
                            <td class="px-6 py-4">
                                <a href="questions.php?delete=<?= $q['id'] ?>" class="text-red-500 hover:text-red-700 ml-2" 
