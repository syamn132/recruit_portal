...
$stmt = $pdo->prepare("
    SELECT c.id, c.name, c.email, c.phone, c.dob, c.verified, c.created_at
    FROM candidates c
");
$stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['upload_offer'])) {
    ...
    $ctc = $_POST['ctc'];
    $monthly_inhand = $_POST['monthly_inhand'];
    ...
    if (move_uploaded_file($_FILES["offer_file"]["tmp_name"], $targetFile)) {
        $stmt = $pdo->prepare("
            INSERT INTO offer_letters 
            (candidate_id, file_path, ctc, monthly_inhand, joining_date)
            VALUES (?, ?, ?, ?, ?)
        ");
        if ($stmt->execute([$candidate_id, $targetFile, $ctc, $monthly_inhand, $joining_date])) {
            $_SESSION['success'] = "Offer letter uploaded successfully!";
        } else {
            $_SESSION['error'] = "Failed to save offer details!";
        }
    } else {
...
<html
  ...
  <head>
    ...
    <style>
      ...
      /* Custom overlay styles for the modal */
    #offerFormModal.custom-overlay {
        position: fixed;
        top: 0;
        left: 0;
        width: 100%;
  ...
  <body>
        ...
        <div class="layout-page">
          ...
          <!-- Content wrapper -->
            <div class="content-wrapper">
                      ...
                      <table class="table table-hover">
                        <thead>
                          <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase" onclick="sortTable(2)">Mobile</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase" onclick="sortTable(3)">DOB</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase" onclick="sortTable(4)">Verified</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase" onclick="sortTable(5)">Joined Date</th>
                        ...
                        <tbody class="divide-y divide-gray-200" id="tableBody">
                          ...
                          <tr>
                            <td class="px-6 py-4"><?= htmlspecialchars($candidate['name']) ?></td>
                            <td class="px-6 py-4"><?= htmlspecialchars($candidate['email']) ?></td>
                            <td class="px-6 py-4"><?= htmlspecialchars($candidate['phone']) ?></td>
                            <td class="px-6 py-4"><?= date('d/m/Y', strtotime($candidate['dob'])) ?></td>
                            <td class="px-6 py-4">
                              <?= $candidate['verified'] ? '✅ Verified' : '❌ Pending' ?>
                            </td>
                            <td class="px-6 py-4"><?= date('M j, Y', strtotime($candidate['created_at'])) ?></td>
                            <td class="px-6 py-4 space-x-2">
                                <a href="javascript:void(0)" onclick="showModal(<?= $candidate['id'] ?>)" class="text-blue-500 hover:text-blue-700">Upload</a>
                            </td>
                          </tr>
                          <?php endforeach; ?>
                        </tbody>
                ...
                <div id="offerFormModal" class="custom-overlay hidden">
                <div class="custom-modal">
                    <h3 class="card-header mb-4">Upload Offer Letter</h3>
                    <form method="POST" enctype="multipart/form-data" action="upload_offer.php">
                        <input type="hidden" name="candidate_id" id="candidate_id"> <!-- Hidden input for candidate ID -->
                        <div class="form-row">
                            <label for="ctc" class="text-gray-700">CTC</label>
                            <input type="number" step="0.01" name="ctc" id="ctc" required>
                        </div>
            
                        <div class="form-row">
                            <label for="monthly_inhand" class="text-gray-700">Monthly In-hand</label>
                            <input type="number" step="0.01" name="monthly_inhand" id="monthly_inhand" required>
                        </div>
                        ...
                        <div class="mb-8">
                            <label for="offer_file" class="form-label">Offer Letter (PDF)</label>
                            <input class="form-control" type="file" name="offer_file" id="offer_file" accept=".pdf" required>
                          </div>
            
                        <div class="form-actions">
                            <button class="btn btn-info mb-1" type="submit" name="upload_offer">Upload Offer Letter</button>
                            <button class="btn btn-danger mb-1" type="button" onclick="closeModal()">Close</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <script>
      ...
      function showModal(candidateId) {
            // Set the candidate ID to the hidden input field
            document.getElementById("candidate_id").value = candidateId;
            document.getElementById("offerFormModal").classList.remove("hidden");
        }

        function closeModal() {
            document.getElementById("offerFormModal").classList.add("hidden");
        }
    </script>
