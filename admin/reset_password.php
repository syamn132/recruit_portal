...
// Handle Reset Password for a candidate
if (isset($_GET['reset'])) {
    $candidate_id = $_GET['reset'];
    try {
        // Generate 8-character temporary password
        $temp_password = bin2hex(random_bytes(4));
        $hashed_temp = password_hash($temp_password, PASSWORD_DEFAULT);
        
        // Update password for the selected candidate
        $stmt = $pdo->prepare("
            UPDATE candidates 
        ...
        if ($stmt->execute([$hashed_temp, $candidate_id])) {
            $_SESSION['success'] = "Temporary password generated: $temp_password";
        } else {
...
<html
  ...
  <head>
    ...
    <title>Reset Password | SoT</title>
        ...
        <tbody class="divide-y divide-gray-200" id="tableBody">
          ...
          <tr>
            ...
            <td class="px-6 py-4 space-x-2">
              <a href="reset_password.php?reset=<?= $candidate['id'] ?>" class="text-red-500 hover:text-red-700" onclick="return confirm('Are you sure you want to reset password for this candidate?')">Reset</a>
            </td>
          </tr>
          <?php endforeach; ?>
        </tbody>
